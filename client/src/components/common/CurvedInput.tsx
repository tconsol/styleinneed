...
const SHADOWS: Record<'sm' | 'md' | 'lg', [number, number, number]> = {
  sm: [5, 12, 0.3],
  md: [10, 24, 0.4],
  lg: [16, 40, 0.52],
};
...
const fmt = (g: Geometry, u: number, v: number) => {
  const [x, y] = g.point(u, v);
  return `${round2(x)} ${round2(y)}`;
};

// Segment along a constant-v edge, as a circular arc (or a line when flat)
const edgeSeg = (g: Geometry, uTo: number, v: number, ltr: boolean) => {
...
const bentRectPath = (g: Geometry, u0: number, u1: number, vTop: number, vBot: number, radius: number) => {
...
const bentLinePath = (g: Geometry, u0: number, u1: number, v: number) => `M ${fmt(g, u0, v)} ${edgeSeg(g, u1, v, true)}`;
...
const CurvedInput = ({
  ...
  const handleSelect = () => {
    ...
    if (input) setCaretIndex(input.selectionStart ?? input.value.length);
  };
  ...
  const handleSurfaceClick = (e: MouseEvent<SVGSVGElement>) => {
    const input = inputRef.current;
    if (!input) return;
  ...
  const shadow = shadowSize === 'none' ? undefined : SHADOWS[shadowSize];
