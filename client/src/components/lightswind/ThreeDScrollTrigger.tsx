...
/* -----------------------------------
   ...
   ----------------------------------- */
const ThreeDScrollTriggerContext = createContext<MotionValue<number> | null>(null);
...
/* --------------------------
   Props
   -------------------------- */
export interface ThreeDScrollTriggerRowProps extends HTMLAttributes<HTMLDivElement> {
  children: ReactNode;
  baseVelocity?: number;
  direction?: 1 | -1;
...
function ThreeDScrollTriggerRowImpl({
  ...
  useAnimationFrame((time) => {
    ...
    // Wrap in both directions so the row loops seamlessly
    if (newX >= unitWidth) {
